...
function CategoryComp(props) {
  ...
  const columns = [
      ...
      renderCell: (params) => (
        <>
          {parseInt(params.row.parent_id) === 0
            ? "Non"
            : params.row.parent.length > 0 &&
            params.row.parent[0].category_nm
          }
        </>
      ),
    },
    {
      ...
      renderCell: (params) => (
        <>
          <IconButton size="small" onClick={() => { setEditCategory(params.row); setCategory(params.row.parent) }}>
  ...
  return (
    <>
        ...
        <Grid item xs={2}>
          <Box sx={{ m: 1 }}>
            <Button
              ...
              fullWidth
              variant="contained"
              onClick={() => { addCategory(login.user_token, category); setValue({ _id: "", label: "" }) }}
            >
              ADD
            </Button>
          </Box>
      ...
      {category.categoryEdit && <EditCategory {...props} categorys={categorys} />}
    </>
  );
}
